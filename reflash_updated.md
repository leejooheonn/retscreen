...
### Headless SD card setup (before first boot)

If flashing manually (not via Raspberry Pi Imager's SSH toggle) and connecting over network
without a password (e.g. open public wifi), the following edits are needed on the boot
partition **before** ejecting the SD card:

- `config.txt` — add `dtoverlay=dwc2` to the end of the file (enables SSH-over-USB / makes
  the gadget usable early, useful when WiFi has no password prompt to authenticate through)
- `cmdline.txt` — after `rootwait`, add a space then paste:
  `modules-load=dwc2,g_ether`
- Add a blank file named exactly `ssh` (no extension) to the boot partition to enable SSH:
  ```bash
  touch /Volumes/bootfs/ssh
  ```

Then eject the card, let the Pi boot, and connect with:
```bash
ssh retinex@retinex.local
```

> Note: `g_ether` in `modules-load` is only needed for this initial bring-up. It must be
> **removed** later (see Step 5 troubleshooting / Step 6) once the custom NCM gadget script
> takes over — otherwise it hijacks the USB controller before `setup_usb_gadget.sh` can run.
...
> Before this step, turn on Internet Sharing on your laptop to share internet with the Pi
> (needed for `apt update`/`apt install` to reach the network).
...
### 5d. Troubleshooting — "USB port already in use" / gadget won't bind

This happens because an older **default** USB driver (loaded via `modules-load=dwc2,g_ether`
from the headless setup step) grabs the USB controller before the custom gadget script can.

Check for the hijacker:
```bash
lsmod | grep -E "^g_"
```
If this shows `g_ether`, `g_serial`, or `g_mass_storage`, that's the culprit. Drop it:
```bash
sudo rmmod g_ether   # or whatever module showed up
```
Clear whatever is currently bound to the USB controller:
```bash
sudo sh -c 'echo "" > /sys/kernel/config/usb_gadget/*/UDC'
```

> On macOS, Bonjour will resolve `retinex.local` over the USB connection too — once the
> gadget is up you don't need the `192.168.7.1` IP for basic `.local` resolution, though the
> static-IP NCM setup below is still needed for reliable networking.

### 5e. Troubleshooting — retinex-gadget.service fails to start

Check the error:
```bash
sudo systemctl status retinex-gadget.service
```

If the error is **"Device or resource busy"**, edit `setup_usb_gadget.sh` and add this line:
```bash
...
Then remove the legacy USB network driver from boot so it stops competing with the custom
script — edit `cmdline.txt`:
```bash
sudo nano /boot/firmware/cmdline.txt
```
and remove `g_ether` from `modules-load=dwc2,g_ether` (leave just `modules-load=dwc2`).
...
## Step 6 — Windows/macOS NCM Network Setup ✅ DONE

Pi USB NCM up at `192.168.7.1/24`. Host NCM adapter set to static IP:
- IP address: `192.168.7.2`
- Subnet mask: `255.255.255.0`
- Default gateway: *(blank)*

**On macOS:** System Settings → Network → select "Retinex Camera" in the sidebar → Details →
TCP/IP tab → change *Configure IPv4* from **Using DHCP** to **Manually** → set IP address to
`192.168.7.2`, subnet mask `255.255.255.0`, router left blank.
...
After connecting, turn **Internet Sharing** back on and make sure it's enabled for the
**"Retinex Camera"** interface, then on the Pi tell it to route outbound traffic back through
the host over USB rather than trying to use the USB wire directly as its own uplink:
```bash
sudo ip route add default via 192.168.7.2 dev usb0
```
